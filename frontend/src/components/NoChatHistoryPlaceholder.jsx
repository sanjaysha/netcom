...
const NoChatHistoryPlaceholder = ({ name, onSuggestionSelect }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-6">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        <button
          type="button"
          onClick={() => onSuggestionSelect("👋 Say Hello")}
        ...
        <button
          type="button"
          onClick={() => onSuggestionSelect("🤝 How are you?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors"
        >
          🤝 How are you?
        </button>
        <button
          type="button"
          onClick={() => onSuggestionSelect("📅 Meet up soon?")}
          className="px-4 py-2 text-xs font-medium text-cyan-400 bg-cyan-500/10 rounded-full hover:bg-cyan-500/20 transition-colors"
        >
          📅 Meet up soon?
        </button>
